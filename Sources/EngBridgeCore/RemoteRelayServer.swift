...
public final class RemoteRelayServer: @unchecked Sendable {
    ...
    peer.start(payload: { [weak self] result in
      ...
      case .failure(let error):
        FileHandle.standardError.write(
          Data("Remote relay connection failed: \(error.localizedDescription)\n".utf8))
        self?.disconnect()
      }
    })
  }
  ...
  public func send(_ envelope: BridgeEnvelope, to peerName: String) throws {
    guard let deviceID = Self.deviceID(from: peerName),
      ...
      throw BridgeError(
        code: "relay_disconnected", message: "The remote relay link is unavailable.",
        recoverable: true)
    }
  ...
  private func receive(_ data: Data) {
    do {
      ...
      if let hello = try? decoder.decode(DirectPairingHello.self, from: data) {
        ...
        let now = Date()
        let expires = now.addingTimeInterval(12 * 60 * 60)
        registry.install(
          try DirectPairingKeyAgreement.bootstrap(
      ...
      guard let bootstrap = registry.credential(for: packet.deviceID) else {
  ...
  private func disconnect() {
    let deviceID = lock.withLock { () -> UUID? in
      defer { connectedDeviceID = nil }
      return connectedDeviceID
    }
    if let deviceID {
      lock.withLock { stateHandler }?("relay:\(deviceID.uuidString)", .disconnected)
    }
  }
